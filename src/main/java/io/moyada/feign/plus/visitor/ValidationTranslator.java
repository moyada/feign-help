...
import io.moyada.feign.plus.annotation.Valid;
import io.moyada.feign.plus.regulation.*;
import io.moyada.feign.plus.support.*;
...
public class ValidationTranslator extends BaseTranslator {
    ...
    private CheckData getThrow(Symbol symbol) {
        ...
        if (null != TreeUtil.getAnnotation(symbol, Valid.class)) {
            return new CheckData(IllegalArgumentException.class.getName(), ElementOptions.MESSAGE);
        }
        return null;
    }
